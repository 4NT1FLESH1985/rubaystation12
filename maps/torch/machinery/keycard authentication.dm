/obj/machinery/keycard_auth/torch/interact(mob/user)
	...
	dat += "<meta charset='utf-8'><h1>Keycard Authentication Device</h1>"

	dat += "This device is used to trigger some high security events. It requires the simultaneous swipe of two high-level ID cards."
	dat += "<meta charset='utf-8'><br><hr><br>"
	...
	if(screen == 1)
		...
		if(security_state.current_security_level == security_state.severe_security_level)
			...
			if(security_state.current_security_level == security_state.high_security_level)
				dat += "<meta charset='utf-8'><A href='?src=\ref[src];triggerevent=Revert alert'>Disengage [security_state.high_security_level.name]</A><br>"
			else
				dat += "<meta charset='utf-8'><A href='?src=\ref[src];triggerevent=Red alert'>Engage [security_state.high_security_level.name]</A><br>"

		if(!config.ert_admin_call_only)
			dat += "<meta charset='utf-8'><A href='?src=\ref[src];triggerevent=Emergency Response Team'>Emergency Response Team</A><br>"

		dat += "<meta charset='utf-8'><A href='?src=\ref[src];triggerevent=Grant Emergency Maintenance Access'>Grant Emergency Maintenance Access</A><br>"
		dat += "<meta charset='utf-8'><A href='?src=\ref[src];triggerevent=Revoke Emergency Maintenance Access'>Revoke Emergency Maintenance Access</A><br>"
		dat += "<meta charset='utf-8'><A href='?src=\ref[src];triggerevent=Grant Nuclear Authorization Code'>Grant Nuclear Authorization Code</A><br>"
		dat += "<meta charset='utf-8'><a href='?src=\ref[src];triggerevent=Evacuate'>Initiate Evacuation Procedures</a><br>"
	if(screen == 2)
		dat += "Please swipe your card to authorize the following event: <b>[event]</b>"
		dat += "<meta charset='utf-8'><p><A href='?src=\ref[src];reset=1'>Back</A>"
