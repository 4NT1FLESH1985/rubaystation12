...
/datum/category_item/player_setup_item/background/culture/content()
	...
	for(var/token in tokens)
		...
		if (expanded[token])
			...
			. += "<meta charset='utf-8'><table width=100%><tr><td colspan=3>"
			for (var/V in valid_values)
				...
				if (pref.cultural_info[token] == V)
					. += "<meta charset='utf-8'><span class='linkOn'>[V]</span> "
				else
					. += "<meta charset='utf-8'><a href='?src=\ref[src];set_token_entry_[token]=[sanitized_value]'>[V]</a> "
