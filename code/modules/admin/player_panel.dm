...
/datum/admins/proc/check_antagonists()
	if (GAME_STATE >= RUNLEVEL_GAME)
		var/dat = list()
		dat += "<meta charset='utf-8'><html><meta charset='utf-8'><head><title>Round Status</title></head><body><h1><B>Round Status</B></h1>"
		dat += "Current Game Mode: <B>[SSticker.mode.name]</B><BR>"
		dat += "Round Duration: <B>[roundduration2text()]</B><BR>"
		dat += "<meta charset='utf-8'><B>Evacuation</B><BR>"
		if (evacuation_controller.is_idle())
			dat += "<meta charset='utf-8'><a href='?src=\ref[src];call_shuttle=1'>Call Evacuation</a><br>"
		else
			var/timeleft = evacuation_controller.get_eta()
			if (evacuation_controller.waiting_to_leave())
				dat += "ETA: [(timeleft / 60) % 60]:[add_zero(num2text(timeleft % 60), 2)]<BR>"
				dat += "<meta charset='utf-8'><a href='?src=\ref[src];call_shuttle=2'>Send Back</a><br>"

		dat += "<meta charset='utf-8'><a href='?src=\ref[src];delay_round_end=1'>[SSticker.delay_end ? "End Round Normally" : "Delay Round End"]</a><br>"
