...
/mob/living/carbon/human/show_inv(mob/user as mob)
	...
	for(var/entry in species.hud.gear)
		...
		dat += "<meta charset='utf-8'><BR><B>[slot_ref["name"]]:</b> <a href='?src=\ref[src];item=[slot_ref["slot"]]'>[istype(thing_in_slot) ? thing_in_slot : "nothing"]</a>"
		if(istype(thing_in_slot, /obj/item/clothing))
			var/obj/item/clothing/C = thing_in_slot
			if(C.accessories.len)
				dat += "<meta charset='utf-8'><BR><A href='?src=\ref[src];item=tie;holder=\ref[C]'>Remove accessory</A>"
	dat += "<meta charset='utf-8'><BR><HR>"

	if(species.hud.has_hands)
		dat += "<meta charset='utf-8'><BR><b>Left hand:</b> <A href='?src=\ref[src];item=[slot_l_hand]'>[istype(l_hand) ? l_hand : "nothing"]</A>"
		dat += "<meta charset='utf-8'><BR><b>Right hand:</b> <A href='?src=\ref[src];item=[slot_r_hand]'>[istype(r_hand) ? r_hand : "nothing"]</A>"

	// Do they get an option to set internals?
	if(istype(wear_mask, /obj/item/clothing/mask) || istype(head, /obj/item/clothing/head/helmet/space))
		if(istype(back, /obj/item/tank) || istype(belt, /obj/item/tank) || istype(s_store, /obj/item/tank))
			dat += "<meta charset='utf-8'><BR><A href='?src=\ref[src];item=internals'>Toggle internals.</A>"

	var/obj/item/clothing/under/suit = w_uniform
	// Other incidentals.
	if(istype(suit))
		dat += "<meta charset='utf-8'><BR><b>Pockets:</b> <A href='?src=\ref[src];item=pockets'>Empty or Place Item</A>"
		if(suit.has_sensor == 1)
			dat += "<meta charset='utf-8'><BR><A href='?src=\ref[src];item=sensors'>Set sensors</A>"
		if (suit.has_sensor && user.get_multitool())
			dat += "<meta charset='utf-8'><BR><A href='?src=\ref[src];item=lock_sensors'>[suit.has_sensor == SUIT_LOCKED_SENSORS ? "Unl" : "L"]ock sensors</A>"
	if(handcuffed)
		dat += "<meta charset='utf-8'><BR><A href='?src=\ref[src];item=[slot_handcuffed]'>Handcuffed</A>"

	for (var/obj/aura/web/W in auras)
		dat += "<meta charset='utf-8'><BR><A href='?src=\ref[src];webbed=1'>Webbed</A>"
		break

	for(var/entry in worn_underwear)
		var/obj/item/underwear/UW = entry
		dat += "<meta charset='utf-8'><BR><a href='?src=\ref[src];item=\ref[UW]'>Remove \the [UW]</a>"

	dat += "<meta charset='utf-8'><BR><A href='?src=\ref[src];refresh=1'>Refresh</A>"
	dat += "<meta charset='utf-8'><BR><A href='?src=\ref[user];mach_close=mob[name]'>Close</A>"
