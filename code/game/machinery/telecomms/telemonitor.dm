	...
	attack_hand(mob/user as mob)
		...
		switch(screen)
		  ...
		  // --- Main Menu ---
			...
			if(0)
				dat += "<meta charset='utf-8'><br>[temp]<br><br>"
				dat += "<meta charset='utf-8'><br>Current Network: <a href='?src=\ref[src];network=1'>[network]</a><br>"
				if(machinelist.len)
					dat += "<meta charset='utf-8'><br>Detected Network Entities:<ul>"
					for(var/obj/machinery/telecomms/T in machinelist)
						dat += "<meta charset='utf-8'><li><a href='?src=\ref[src];viewmachine=[T.id]'>\ref[T] [T.name]</a> ([T.id])</li>"
					dat += "<meta charset='utf-8'></ul>"
					dat += "<meta charset='utf-8'><br><a href='?src=\ref[src];operation=release'>\[Flush Buffer\]</a>"
				else
					dat += "<meta charset='utf-8'><a href='?src=\ref[src];operation=probe'>\[Probe Network\]</a>"
		  ...
		  // --- Viewing Machine ---
			...
			if(1)
				...
				dat += "<meta charset='utf-8'><center><a href='?src=\ref[src];operation=mainmenu'>\[Main Menu\]</a></center>"
				dat += "<meta charset='utf-8'><br>Current Network: [network]<br>"
				dat += "Selected Network Entity: [SelectedMachine.name] ([SelectedMachine.id])<br>"
				dat += "Linked Entities: <ol>"
				for(var/obj/machinery/telecomms/T in SelectedMachine.links)
					if(!T.hide)
						dat += "<meta charset='utf-8'><li><a href='?src=\ref[src];viewmachine=[T.id]'>\ref[T.id] [T.name]</a> ([T.id])</li>"
